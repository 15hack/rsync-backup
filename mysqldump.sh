...
ssh rhetzner "pct exec 101 -- bash -c 'grep password /etc/mysql/debian.cnf | head -n 1 | sed \"s/.* = //\" | pt-show-grants --user debian-sys-maint --ask-pass' 2>/dev/null"  | gzip -9 > "mysql/users_grants.sql.gz"
for DB in "${DBS[@]}"; do
